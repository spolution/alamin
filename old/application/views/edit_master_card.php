	...
	<table class="input">
		<tr>
				...
				<select name='kode_kartu' id='kode_kartu'>
					...
					<?php
						...
						foreach ($kartu->result() as $row){
							if ($row->kode_kartu == $fetch->kode_kartu) {
								$pro = 'selected';
							}else{
								$pro = null;
							}
							
							echo "<option value='".$row->kode_kartu."' ".$pro.">".$row->nama_kartu."</option>";
						}
			...
			<td>
				...
				<?=form_input(array('name'=>'hrg_dasar','id'=>'hrg_dasar','value'=>$fetch->hrg_dasar));?>
			</td>
		</tr>
		<tr>
			<td>
				<label for="kode_mc">Kode Master Card (harus diisi)</label><br />
				<?=form_input(array('name'=>'kode_mc','id'=>'kode_mc','value'=>$fetch->kode_mc));?>
			</td>
			<td>
				<label for="hrg_jual">Harga Jual (harus diisi)</label><br />
				<?=form_input(array('name'=>'hrg_jual','id'=>'hrg_jual','value'=>$fetch->hrg_jual));?>
			</td>
		</tr>
		<tr>
			<td>
				...
				if ($type == 1) {
					echo "
						<td>
							...
							form_input(array('name'=>'saldo_satuan','id'=>'saldo_satuan','value'=>$fetch->saldo_satuan))
						."</td>
					";
				}
		...
		<tr>
			<td>
				...
				<?=form_input(array('name'=>'ket','id'=>'ket','value'=>$fetch->ket));?>
			</td>
			<td>
				<?=form_submit(array('name'=>'edit','class'=>'radius','value'=>'edit data'))
